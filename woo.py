...
class woo(Exchange):
    ...
    def fetch_borrow_rate_history(self, side=None, token=None, since=None, limit=None, params={}):
        self.load_markets()
        request = {}
        market = None
        if token is not None:
            market = self.market(token)
            request['token'] = market['id']
        if since is not None:
            request['start_t'] = since
        response = self.v1PrivateGetInterestHistory(self.extend(request, params))
        result = self.safe_value(response, 'rows', [])
        return self.parse_incomes(result,since,limit)
    ...
    def fetch_borrow_rates(self, params={}):
        response = self.v1PrivateGetTokenInterest(params)
        rows = self.safe_value(response, 'rows', [])
        return self.filter_by_array(rows, 'token')
